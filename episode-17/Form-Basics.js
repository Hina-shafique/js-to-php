// Event Basics

class Game {
  #minRange; // private
  #maxRange; // private
  #maxAttempts; // private

  constructor({ minRange = 1, maxRange = 10, maxAttempts = 3 } = {}) {
    this.#minRange = Game.initRangeValues({
      value: minRange,
      lowerBounds: 0,
      upperBounds: maxRange,
    });

    this.#maxRange = Game.initRangeValues({
      value: maxRange,
      lowerBounds: minRange,
    });

    this.#maxAttempts = maxAttempts;
  }

  get minRange() {
    return this.#minRange;
  }

  set minRange(value) {
    this.#minRange = Game.initRangeValues({
      value,
      lowerBounds: 0,
      upperBounds: this.#maxRange,
    });
  }

  get maxRange() {
    return this.#maxRange;
  }

  set maxRange(value) {
    this.#maxRange = Game.initRangeValues({
      value,
      lowerBounds: this.#minRange,
    });
  }

  get maxAttempts() {
    return this.#maxAttempts;
  }

  set maxAttempts(value) {
    this.#maxAttempts = value;
  }

  static initRangeValues({ value, lowerBounds, upperBounds = 0 } = {}) {
    let num = Number(value);

    if (isNaN(num)) {
      throw {
        message: "Value must be numeric",
      };
    }

    if (num < lowerBounds) {
      throw {
        message: `Value cannot be less than ${lowerBounds}`,
      };
    }

    if (upperBounds && num > upperBounds) {
      throw {
        message: `Value cannot be greater than ${upperBounds}`,
      };
    }

    return num;
  }

  checkGuess(guess) {
    if (guess === this.secretNumber) {
      console.log("Congrats! You guessed the number");
      return true;
    } else if (guess < this.secretNumber) {
      console.log(`${guess} is too low.`);
    } else {
      console.log(`${guess} is too high.`);
    }

    return false;
  }

  play() {
    this.secretNumber =
      Math.floor(Math.random() * (this.#maxRange - this.#minRange + 1)) +
      this.#minRange;
    const history = [];

    while (history.length < this.#maxAttempts) {
      var input = prompt("Please enter a number between 1 and 10");

      if (input === null) {
        break;
      }

      var guess = Number(input);

      if (isNaN(guess) || guess < this.#minRange || guess > this.#maxRange) {
        console.log("Please enter a valid number from 1 and 10");
        continue;
      }

      if (history.indexOf(guess) > -1) {
        continue;
      }

      history.push(guess);

      if (this.checkGuess(guess)) {
        var guessed = true;
        break;
      }
    }

    var guessedMessage = guessed ? "guessed" : "didn't guess";

    console.log(
      `Game over! The number is ${this.secretNumber}, and you ${guessedMessage} in ${history.length} attempts`
    );
    console.log(`Guessed numbers are: ${history.join(", ")}`);
  }
}

document
  .getElementById("settings-form")
  .addEventListener("submit", function (e) {
    e.preventDefault();
    const TilteElement = document.getElementById("input-title");
    const minRangeElement = document.getElementById("input-min-range");
    const maxRangeElement = document.getElementById("input-max-range");
    const maxAttemptsElement = document.getElementById("input-max-attempts");

    const submitterName = e.submitter.name;
    if (submitterName === "play-game") {
      const Tilte = TilteElement.value;
      const minRange = minRangeElement.value;
      const maxRange = maxRangeElement.value;
      const maxAttempts = maxAttemptsElement.value;

      if (!Tilte || !minRange || !maxRange || !maxAttempts) {
        console.log("Please fill in all fields");
        return;
      }
      let easyGame = new Game({ minRange, maxRange, maxAttempts });
      easyGame.play();
    } else {
      TilteElement.value = "";
      minRangeElement.value = "";
      maxRangeElement.value = "";
      maxAttemptsElement.value = "";

      console.clear;
    }
  });

document.getElementById("play-game").addEventListener("click", function (e) {
  e.preventDefault();

  const Tilte = document.getElementById("input-title").value;
  const minRange = document.getElementById("input-min-range").value;
  const maxRange = document.getElementById("input-max-range").value;
  const maxAttempts = document.getElementById("input-max-attempts").value;

  let easyGame = new Game({ minRange, maxRange, maxAttempts });
  easyGame.play();
});

document.getElementById("clear-game").addEventListener("click", function (e) {
  e.preventDefault();
  document.getElementById("input-title").value = "";
  document.getElementById("input-min-range").value = "";
  document.getElementById("input-max-range").value = "";
  document.getElementById("input-max-attempts").value = "";

  console.log("Form cleared");
});
